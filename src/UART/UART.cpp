...
void UART_Update() {
  ...
  while (uartSerial->available()) {
    ...
    // Khong reset parser khi gap 0x7E giua goi: payload nhi phan (float, gia tri
    // motor 1150/1406/..., timestamp, seq, CRC) hay chua byte 0x7E, doan reset cu
    // lam mat goi va lech khung 1 byte (byte SOF bi doc thanh "type"). Khung goi
    // da co length + CRC nen chi can tim SOF khi dang o trang thai SOF.
    switch (uartState) {
      case UART_PARSE_SOF:
        if (c == PKT_SOF) {

...
void Flight_Update() {
  ...
  // Yeu cau nav (POSHOLD/RTH/GEOFENCE) ma GPS chua dung duoc thi bo qua yeu cau
  // va tiep tuc giu thang bang o che do thuong (navActive=false nen setpoint
  // van lay tu can gat), khong cat motor.
  ...
  if (millis() - lastLogMs > 200) {
    ...
    // r/p = roll/pitch tuong doi so voi zero-point luc arm, don vi 0.1 do.
    const int rollDeci = static_cast<int>((imu.roll - GetZeroPointRollOffset()) * 10.0f);
    const int pitchDeci = static_cast<int>((imu.pitch - GetZeroPointPitchOffset()) * 10.0f);
    DEBUG_SERIAL.printf("[FLT] a=%d thr=%d aux=%02X imu=%d ok=%d q=%d/%d rc=%d nav=%d/%d r=%d p=%d m=%d,%d,%d,%d\n",
        rc.armed, rc.throttle, rc.aux, imuFresh, imuAccurate,
        imuStatus, threshold, rcFresh,
        navRequested, navActive, rollDeci, pitchDeci,
